...
class SSP {
        ...
        for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
            ...
            for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
                ...
                if ( isset( $column['formatter'] ) ) {
                    $row[ $column['dt'] ] = ($isJoin) ? $column['formatter']( (isset($column['as']) ? $data[$i][ $column['as'] ] : $data[$i][ $column['field'] ]), $data[$i] ) : $column['formatter']( $data[$i][ $column['db'] ], $data[$i] );
                }
                else {
                    //$row[ $column['dt'] ] = ($isJoin) ? $data[$i][ $columns[$j]['field'] ] : $data[$i][ $columns[$j]['db'] ];
					$row[ $column['dt'] ] = ($isJoin) ? (isset($column['as']) ? $data[$i][ $column['as'] ] : $data[$i][ $column['field'] ]) : $data[$i][ $column['db'] ];
                }
            }
    ...
    static function simple ( $request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '', $having = '')
    {
		...
		if ($table=="edificios"){
			$extrawherecontador=($extraWhere) ? ' WHERE '.$extraWhere : '';
		} else {
			$extrawherecontador="";
		}
